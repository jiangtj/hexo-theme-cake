...
module.exports = (ctx) => (data, options) => {
  const siteCfg = Object.assign({}, {
    config: {
      ...
      relative_link: ctx.config.relative_link
    }
  });
  let renderer = new Renderer();
  ctx.execFilterSync('marked:renderer', renderer, {context: ctx});
  return marked(data.text, Object.assign({
    renderer: renderer
  }, ctx.config.marked, options, siteCfg));

  // return Promise.resolve(new Renderer())
  //   .then(renderer => {
  //     return ctx.execFilter('marked:renderer', new Renderer(), {context: ctx});
  //   })
  //   .then(renderer => {
  //     return marked(data.text, Object.assign({
  //       renderer: renderer
